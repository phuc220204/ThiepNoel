...
// Lấy dữ liệu bức thư từ CONFIG (đã loại bỏ API)
function getMessageDataFromApi() {
  try {
    // Đọc từ CONFIG thay vì apiData
    if (window.CONFIG && window.CONFIG.letterContent) {
      const raw = window.CONFIG.letterContent.trim();
    ...
    console.warn("Không đọc được letterContent từ CONFIG, dùng default:", e);
  }
  // Chỉ dùng dữ liệu mặc định nếu không có CONFIG hoặc không có trường letterContent
